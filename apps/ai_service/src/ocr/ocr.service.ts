...
export class OcrService {
  ...
  private async runExtraction(
    ...
    const { prompt, chain } = this.getExtractionPipeline();
    const result = await prompt.pipe(chain).invoke({
  ...
  /**
   * Builds and caches the structured-output chain and OCR prompt on first use.
   * Idle workers avoid creating the Gemini/LangChain pipeline until an OCR job
   * actually needs extraction.
   */
  private getExtractionPipeline(): {
    chain: Runnable<
      BaseLanguageModelInput,
      { raw: BaseMessage; parsed: OcrResult | null }
    >;
    ...
    this.extractionPipeline = { chain, prompt };
    return this.extractionPipeline;
  }
