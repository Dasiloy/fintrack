        ...
        {TIMELINE_ENTRIES.map(({ year, title, description, icon: Icon }, idx) => (
          <li key={year} className="relative flex gap-6 pb-10 last:pb-0">
            ...
            {idx < TIMELINE_ENTRIES.length - 1 && (
              <div
                aria-hidden="true"
                className="border-border-light absolute top-12 left-6 h-[calc(100%-3rem)] w-px -translate-x-1/2 border-l border-dashed"
              />
            )}

            {/* Icon node */}
            <div className="bg-bg-elevated border-border-light text-primary relative z-10 flex size-12 shrink-0 items-center justify-center rounded-full border">
              <Icon size={18} aria-hidden="true" />
            </div>

            {/* Content */}
            <div className="rounded-card bg-bg-elevated border-border-light hover:bg-bg-surface-hover border p-6 flex-1 transition-colors duration-smooth">
