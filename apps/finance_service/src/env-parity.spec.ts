...
describe('Finance service environment files', () => {
  it.each(['.env', '.env.example'])(
    '%s includes every local finance env key',
    (fileName) => {
  ...
  it('validates every required runtime key in FinanceModule', () => {
    const source = readFileSync(join(__dirname, 'finance.module.ts'), 'utf8');
