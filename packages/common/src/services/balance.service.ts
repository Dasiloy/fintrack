...
import { currentMonthBounds, currentMonthYear, toMonthYear } from '@fintrack/utils/date';
import { Prisma, TransactionType, UserBalance } from '@fintrack/database/types';
...
export class BalanceService {
  ...
  async ensureCurrentMonth(
    ...
    if (!existing || existing.monthYear === thisMonth) return existing;
  ...
  async applyBalanceDelta(
    ...
    if (isPastMonth) {
      await tx.monthlyBalanceSnapshot.upsert({
        ...
        update: {
          ...(isIncome ? { income: { increment: delta } } : { expense: { increment: delta } }),
